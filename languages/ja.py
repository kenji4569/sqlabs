# coding: utf8
{
'%Y-%m-%d': '%Y-%m-%d',
'%Y-%m-%d %H:%M:%S': '%Y-%m-%d %H:%M:%S',
'%s is a powerful opensource web framework based on python programming language.\nHere we love to share useful code parts derived from the development using this framework.\nThe code parts are organized in %s, and easily available.': '%s は強力なオープンソースのウェブ・フレームワークです。プログラミング言語はPythonを使用しています。ここでは、このフレームワークを用いた開発から派生した有用なコード部品を共有したいと思います。コード部品は %s に基づいて整理されいて、簡単に利用可能です。',
'All': 'すべて',
'Are you sure you want to delete this object?': 'このオブジェクトを削除してもよろしいですか?',
'Back to the list': '一覧に戻る',
'Cloudmap is a visual search engine for any contents with user evaluations.': 'Cloudmap is a visual search engine for any contents with user evaluations.',
'Demo': 'デモ',
'Display: <b>%(start)s - %(end)s</b> of <b>%(total)s</b>': '全<b>%(total)s</b>件 (<b>%(start)s ～ %(end)s</b>件目)',
'Introducing new products or services being developed by %s': '%s で開発している新商品やサービスを紹介します',
'Next': '>',
'Paginate: ': '1ページに:',
'Prev': '<',
'See Demo': 'デモを見る',
'Source code': 'ソースコード',
'Static files': '静的ファイル',
'There are no static files': '静的ファイルはありません',
'Usage': '使用例',
"a web2py's plugin system": 'web2pyのプラグインシステム',
'delete': '削除',
'enter an integer between %(min)g and %(max)g': '%(min)g ～ %(max)g の整数を入力してください',
'enter an integer greater than or equal to %(min)g': '%(min)g 以下の整数を入力してください',
'register': '登録',
'under construction': '準備中',
}
