...
class TestCommentBox(unittest.TestCase):
    ...
    def test_crud(self):
        for target_id in range(1, 3):
            ...
            self.assertEqual(comment_box.comments_from_target(target_id).select().first().body, 'body_1')
            
            comment_box.add_comment(user_id, target_id, 'body_2')
            self.assertEqual(comment_box.comments_from_target(target_id).count(), 2)
            
            user_id = 2
            comment_box.add_comment(user_id, target_id, 'body_3')
            
            comments = comment_box.comments_from_target(target_id).select()
